...
Implementation findings: macOS commands exec the exact interpreter directly to avoid CPython 3.11.1's venv prefix bug when launched through an out-of-directory symlink. Windows puts the exact runtime directory and its Scripts directory on PATH, keeping executable aliases beside their DLLs. Windows preserves registry PATH value types and unexpanded variables. UAC elevation must retain the initiating account's SID; cross-account elevation fails before configuration. Unsupported legacy ANSI profile encodings fail without modifying their bytes.
